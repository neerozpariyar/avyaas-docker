...
func (handler *handler) UpdateLiveGroup() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		// Invoke request to the liveGroup  usecase to update the liveGroup
		errMap = handler.usecase.UpdateLiveGroup(requestBody)
		if len(errMap) > 0 {
			return c.Status(http.StatusBadRequest).JSON(presenter.ErrorResponse(errMap))
		}
