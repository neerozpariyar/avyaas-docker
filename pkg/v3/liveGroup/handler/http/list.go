...
func (handler *handler) ListLiveGroup() fiber.Handler {
	return func(c *fiber.Ctx) error {
		errMap := make(map[string]string)

		request := presenter.ListLiveGroupRequest{
		...
		liveGroups, totalPage, err := handler.usecase.ListLiveGroup(request)
		if err != nil {
			errMap["error"] = err.Error()
			return c.Status(http.StatusBadRequest).JSON(presenter.ErrorResponse(errMap))
		} else if liveGroups == nil {
