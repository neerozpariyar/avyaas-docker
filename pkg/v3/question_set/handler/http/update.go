...
/*
CreateQuestionSet handles the HTTP request to create a new question set by sending a HTTP request to
the question set usecase.
...
*/
func (handler *handler) UpdateQuestionSet() fiber.Handler {
	return func(c *fiber.Ctx) error {
		errMap := make(map[string]string)
		var requestBody presenter.CreateUpdateQuestionSetRequest
		...
		// Invoke request to the test type usecase to create the question set
		errMap = handler.usecase.UpdateQuestionSet(requestBody)
		if len(errMap) > 0 {
			return c.Status(http.StatusBadRequest).JSON(presenter.ErrorResponse(errMap))
		}
