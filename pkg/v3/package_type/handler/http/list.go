...
func (handler *handler) ListPackageType() fiber.Handler {
	return func(c *fiber.Ctx) error {
		errMap := make(map[string]string)

		request := &presenter.PackageTypeListRequest{
			Page:     utils.CheckPageInQuery(c),
			PageSize: utils.CheckPageSizeInQuery(c),
			Search:   c.Query("search"),
		}

		// Invoke the package type list usecase to retrieve the list of package types
		packageTypes, totalPage, err := handler.usecase.ListPackageType(request)
		if err != nil {
			errMap["error"] = err.Error()
			return c.Status(http.StatusBadRequest).JSON(presenter.ErrorResponse(errMap))
		} else if len(packageTypes) == 0 {
