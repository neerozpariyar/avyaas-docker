...
func (handler *handler) UpdateDiscussion() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		// Invoke request to the discussion  usecase to update the discussion
		errMap = handler.usecase.UpdateDiscussion(requestBody)
		if len(errMap) > 0 {
			return c.Status(http.StatusBadRequest).JSON(presenter.ErrorResponse(errMap))
		}
