...
func (handler *handler) GetTestDetails() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		testDetails, err := handler.usecase.GetTestDetails(testID, requesterID)
		if err != nil {
			return c.Status(http.StatusBadRequest).JSON(fiber.Map{"error": err.Error()})
		}

		return c.JSON(fiber.Map{"data": testDetails})
	}
}
