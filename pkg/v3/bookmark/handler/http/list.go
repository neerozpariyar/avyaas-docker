...
func (handler *handler) ListBookmark() fiber.Handler {
	return func(c *fiber.Ctx) error {
		errMap := make(map[string]string)
		request := &presenter.BookmarkListRequest{
			Page:         utils.CheckPageInQuery(c),
			PageSize:     utils.CheckPageSizeInQuery(c),
			BookmarkType: c.Query("bookmarkType"),
			Search:       c.Query("search"),
			UserID:       c.Locals("requester").(uint),
		}
		bookmarks, totalPage, err := handler.usecase.ListBookmark(*request)
		if err != nil {
