...
func (handler *handler) UpdateCaseBasedQuestion() fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		for q := 0; ; q++ {
			...
			title := c.FormValue(fmt.Sprintf("questions[%d][title]", q))
			...
			for i := 0; i < 4; i++ {
				...
				imageFile, _ = c.FormFile(fmt.Sprintf("questions[%d][options][%d][image]", q, i)) // Ignore error if file is not present
		...
		errMap = handler.usecase.UpdateQuestion(requestBody)
		if len(errMap) > 0 {
			return c.Status(http.StatusBadRequest).JSON(presenter.ErrorResponse(errMap))
