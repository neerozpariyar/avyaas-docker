...
func (handler *handler) ListLive() fiber.Handler {
	return func(c *fiber.Ctx) error {
		errMap := make(map[string]string)

		request := presenter.ListLiveRequest{
			UserID:      c.Locals("requester").(uint),
			CourseID:    uint(utils.StringToUint(c.Query("courseID"))),
			LiveGroupID: uint(utils.StringToUint(c.Query("liveGroupID"))),
			Page:        utils.CheckPageInQuery(c),
			PageSize:    utils.CheckPageSizeInQuery(c),
			Search:      c.Query("search"),
		}

		lives, totalPage, err := handler.usecase.ListLive(request)
		if err != nil {
