...
func (handler *handler) ListStudent() fiber.Handler {
	return func(c *fiber.Ctx) error {
		errMap := make(map[string]string)

		// Check if page value is set in query params
		request := &presenter.StudentListRequest{
		...
		// Invoke the student list usecase to retrieve the list of student
		students, totalPage, err := handler.usecase.ListStudent(request)
		if err != nil {
			errMap["error"] = err.Error()
			return c.Status(http.StatusBadRequest).JSON(presenter.ErrorResponse(errMap))
		} else if students == nil {
