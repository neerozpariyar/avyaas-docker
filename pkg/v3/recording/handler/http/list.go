...
func (handler *handler) ListRecording() fiber.Handler {
	return func(c *fiber.Ctx) error {
		errMap := make(map[string]string)

		request := &presenter.RecordingListRequest{
			Page:     utils.CheckPageInQuery(c),
			PageSize: utils.CheckPageSizeInQuery(c),
			LiveID:   uint(utils.StringToUint(c.Query("liveID"))),
			Search:   c.Query("search"),
		}

		recordings, totalPage, err := handler.usecase.ListRecording(*request)
		if err != nil {
			errMap["error"] = err.Error()
			return c.Status(http.StatusBadRequest).JSON(presenter.ErrorResponse(errMap))
		} else if recordings == nil {
